...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.inplanes != self.outplanes:
        		out = out + self.shortcuts(x)  # Changed from += (in-place) to + (non-in-place)
        else:
        		out = out + x  # Changed from += (in-place) to + (non-in-place)
...
class lightingNet(nn.Module):
    ...
    def forward(self, innerFeat, target_light, count, skip_count):
        ...
        # Fixed: Use non-in-place concatenation instead of assignment
        # Old: innerFeat[:,0:self.ncInput,:,:] = upFeat (causes in-place error)
        # New: Create new tensor with updated features
        updated_feat = torch.cat([upFeat, innerFeat[:,self.ncInput:,:,:]], dim=1)
        return updated_feat, updated_feat[:, self.ncInput:, :, :], light
